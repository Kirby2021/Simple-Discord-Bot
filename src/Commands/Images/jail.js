    ...
    async run(message, args, client) {
         ...
         try {
          const m = await message.reply({content: "Generating image <a:Kao_loading:938867145331339264>", allowedMentions: {repliedUser: false}});
           const buffer = await AmeAPI.generate("jail", { url: user.displayAvatarURL({ format: "png", size: 512 }) });
           const attachment = new Discord.MessageAttachment(buffer, "jail.png");

           m.edit({ content: "Generated <a:Kao_mochaDance:948192364864868362>", files: [attachment],name: "jail.png", allowedMentions: {repliedUser: false}});
         } catch (err) {
